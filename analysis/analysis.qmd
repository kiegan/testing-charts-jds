...
```{r}
#| out-width: 80%
#| fig-cap: "Certainty by task and correctness. Correct answers in the unaligned task have the least certainty associated with them. An incoorect response on the unaligned task shows a significant oost in certainty. "
round_12_certainty_long %>% 
  ggplot(aes(x = Response, weight=weight)) + geom_bar() + 
  ylab("Number of (weighted) responses") + 
  ...
  geom_vline(
    ...
    data = data.frame(
      coefs_certainty, 
      Correct= gsub(".*(FALSE|TRUE)", "\\1", terms),
