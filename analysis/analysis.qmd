...
@fig-response-123 shows the results of a cell-means model with ordinal response $Y$, 

$$
\text{logit }P(Y_k \le y) = \mu_{ij(k)}
$$
where $Y_k$ is the $k$th participant's response with $Y_k \in \{1, 2, 3\}$, where 'correct' is encoded as 1, 'they are the same' is encoded as 2, and 'wrong' is encoded as 3; $i \in \{1, 2\}$ is the comparison type (1 = Aligned, 2 = Unaligned),  and $j \in \{1, 2, 3\}$ is the chart design, with 1 = Vertical, 2 = Horizontal, and 3 = Horizontal wide. 

HH: the definition of this model isn't quite right yet - will fix it in the morning. 
