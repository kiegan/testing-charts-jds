...
From @zacksReadingBarGraphs1998a a good explanation why the results from the facetted barcharts are so bad: we are running into a context-induced illusion. Lines are perceived to be bigger, if there is a bigger line next to them. -- Here, we see the opposite effect.

> The graphical context in which an element occurs can produce distortions in judgments of color, angle, size, and orientation (see, e.g., Goldstein, 1989, especially chap. 7; Howard, 1982). One particularly relevant example of such a distortion is given in Figure 1. This figure demonstrates the parallel lines illusion: When two parallel lines are viewed, the viewer tends to perceive assimilation (the lengths of the lines seem closer than they are) or contrast (the lengths of the lines seem more different than they are), depending on the ratio of the line lengths and the distance between them (Jordan & Schiano, 1986; Schiano, 1986).
...
response_67 %>%
  ...
  ggplot(aes(y = task, x = time_spent, fill=task)) + 
  geom_boxplot() +
...
$$
where  $\ell \in \{1, 2\}$; $i \in \{1, 2\}$ is the comparison type (1 = Aligned, 2 = Unaligned),  and $j \in \{1, 2, 3\}$ is the chart design, with 1 = Vertical, 2 = Horizontal, and 3 = Horizontal wide. The estimated values and 95% confidence intervals are shown in @tbl-rounds-123
...
A contributing factor to this outcome might be the way that participants interact with the different designs. Generally, about half of all participants make use of the option to zoom into charts - while zooming does help with the overall accuracy (which is in agreement with the findings by @luModelingJustNoticeable2022 about the physical size of stimuli), the increase is not significant.  However, different designs lead to different rates of zooming: when dealing with the vertical design, the rate of zooming is significantly higher than for the two horizontal designs.
...
Let $Y_{jk}$ describe the zooming behavior of panelist $k$ on task $j$. We model zooming behavior (no = 0, yes = 1) as a logistic regression by correctness of response ($\rho$),  task ($\tau$), and design ($\delta$) of the chart:
$$
...
coefs_zoom$term <- c("$$\\hat{\\mu}$$", "They are the same $$\\widehat{\\rho}_{2}$$", 
...
## Model 4: Unaligned Elements in horizontal wide barcharts compared to floating bars

```{r}
#| label: fig-context-bars
#| fig-cap: Floating bars show and horizontal wide barcharts show the same areas  with the same distances
#| fig-subcap: 
#|   - floating bars
#|   - horizontal wide
#| layout-nrow: 1
#| out-width: 100%
knitr::include_graphics("images/floating - all.PNG")
knitr::include_graphics("images/round3/hwide-ab.PNG")
```
...
```{r combine-r35}
# wide horizontal
wide_horiz <- round3_adj %>% filter(design=="Horizontal wide")
  
# floating bars are in round5
lambda <- calculate_lambda(wide_horiz$weight, round5_adj$weight)

round_35 <- rbind(
  wide_horiz %>% mutate(
    weight = lambda*weight/sum(weight),
    task = "horizontal wide"
  ) %>% dplyr::select(CaseId, weight, ab, ab_zoom, ab_certain, task),
  round5_adj %>% mutate(
    weight = (1-lambda)*weight/sum(weight),
    task = "floating"
  ) %>% dplyr::select(CaseId, weight, ab, ab_zoom, ab_certain, task)
) %>% mutate(
  correct = ab=="B is bigger",
  not_wrong = ab != "A is bigger",
  task = factor(task)
) 

survey_35 <- svydesign(ids=~CaseId, weights=~weight, data = round_35)
```

```{r model-r35}
logit_35_correct <- svyglm(correct~task-1, design = survey_35, family=quasibinomial())
logit_35_not_wrong <- svyglm(not_wrong~task-1, design = survey_35, family=quasibinomial())

coefs_35_correct <- broom::tidy(logit_35_correct) %>% 
  mutate(task = factor(gsub("task", "", term), levels=levels(round_35$task)))
coefs_35_notwrong <- broom::tidy(logit_35_not_wrong) %>% 
  mutate(task = factor(gsub("task", "", term), levels=levels(round_35$task)))
```


```{r}
#| label: fig-round-floats
#| fig-cap: "Accuracy of responses by design. Free floating bars are comparisons of lengths. The additional context of stacked barcharts anchors the bars for a significant  increase in correct responses. However, the percentage of wrong responses also significantly increases. "
#| out-width: 70%

round_35 %>% 
  mutate(
    response3 = c("Wrong", "They are the same", "Correct")[correct+not_wrong+1],
    response3 = factor(response3, levels=rev(c("Correct", "They are the same", "Wrong")))
  ) %>% 
  ggplot(aes(x = task)) + 
  geom_bar(aes(fill = response3, weight = weight), position="fill") +
#  facet_grid(.~task) + 
  ...
  scale_fill_manual("Response", values=c("orange", "grey", "steelblue")) +
  xlab("") +
  scale_y_continuous("Percent responses", breaks=seq(0,1,by=0.25), labels = 100*seq(0,1,by=0.25)) + 
  geom_rect(
    aes(
      xmin = as.numeric(task) - 0.45,
      ymin = inv_logit(estimate - 1.96* std.error),
      xmax = as.numeric(task) + 0.45,
      ymax = inv_logit(estimate + 1.96* std.error),
    ), 
    fill = "black", alpha = 0.25, #colour = "white", linewidth = 0.25,
    data = coefs_35_correct
  ) +
  geom_segment(
    aes(
      x = as.numeric(task) - 0.45,
      y = inv_logit(estimate ),
      xend = as.numeric(task) + 0.45,
      yend = inv_logit(estimate )
    ), 
    alpha = 1, colour = "black", linewidth = 0.25,
    data = coefs_35_correct
  ) +
  ...
  ggtitle("Comparison of unaligned tiles")
#+ 
#  geom_text(aes(label=letters), y = -0.05, data = ci_confint, colour = "steelblue") +
#  geom_text(aes(label=letters), y = 1.05, data = ci_confint3, colour = "darkorange") 
...
```{r m35-summary}
#| label: tbl-floating
#| tbl-cap: "Percentage of accuracy of responses by design."
coefs_35 <- coefs_35_notwrong %>% mutate(
  level = c("correct or same | wrong")
) %>% rbind(
coefs_35_correct %>% mutate(
  level = c("correct | same or wrong")
))
...
coefs_35 %>% 
  mutate(
    ...
    perc_ci = sprintf(" %.2f [%.2f, %.2f]", estimate, ci_low, ci_high)
    # perc_ci = sprintf(" %.2f [%.2f, %.2f] %s", estimate, ci_low, ci_high,
    #                    ifelse(p.value < 0.001, "***",
    #                      ifelse(p.value < 0.01, "**",
    #                        ifelse(p.value < 0.05, "*",
    #                          ifelse(p.value < 0.1, ".", " ")))))
  ) %>% 
  dplyr::select(level, task, perc_ci) %>%
  pivot_wider(names_from="task", values_from="perc_ci") %>%
  gt() %>%
  tab_header("Estimated percentage of responses [95% CI]") %>%
  cols_label(
    "level" = " "
  )
```
...
How do our findings relate to @cejaTruthSquareAspect2021 ?

Science communication review of visuals: @desnoyersTaxonomyVisualsScience2023
