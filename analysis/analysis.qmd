...
$$
\text{ logit } P(y_k \le i) = \mu_i + \alpha
$$

```{r}
#| label: fig-demographics
#| fig-cap: "Accuracy across demographics. Gender and age are not significant factors. However, educational level and income do have -- a small -- effect.  When income levels increase, the percentage of wrong answers decreases. 
#| With increasing levels in education, we see a small drop in correct answers, as well as a small drop in wrong answers, for a significant increase in 'they are the same'."
#| fig-height: 4.25
#| fig-width: 7
#| out-width: 100%
...
gender + age +income + educ + plot_layout(nrow=1)
```

```{r demographics}
logit_demo <- svyglm(correct~gender+educ5+age4+income4, design=survey_123, family=quasibinomial())
logit_demo3 <- svyglm(correct3~gender+educ5+age4+income4, design=survey_123, family=quasibinomial())
