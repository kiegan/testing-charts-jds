...
|---------|-------|--------:|----------:|
| Round 1 | April 2022 |  942    |    `r round(sum(round1_adj$weight),1)`       |
| Round 2 | May 2022 |  960   |    `r round(sum(round2_adj$weight),1)`       |
|         |       |         |           |
...
Accuracy comparing aligned versus unaligned bars. The data used for this is a combination of rounds 1 and 2, with $\lambda$ = `r round(lambda[1],3)` for an effective sample size of `r round(sum(round_12_long$weight/2),1)`. \autoref{fig-alignment}(a) shows that more than twice the number of responses is accurate, when the tiles are aligned along the same axis. 
