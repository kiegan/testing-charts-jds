...
age_ab <- round_123_long %>% 
 ...
 geom_bar(aes(fill = response3, weight = weight), position="fill") +
  ...
  xlab("") +
    ggtitle("\nby age") +
  scale_y_continuous("Percentage", breaks=seq(0,1, by=0.25), labels = seq(0,1, by=0.25)*100)
gender_cd + age_cd +income_cd + educ_cd + plot_layout(nrow=1, guides = "collect")

gender_ab + age_ab +income_ab + educ_ab + plot_layout(nrow=1, guides = "collect")
```

```{r demographics-model}
logit_demo_task_cd_correct <- svyglm(correct~gender+educ5+age4+income4, design=survey_123, family=quasibinomial(), subset = task=="cd")
logit_demo_task_cd_correct_same <- svyglm(correct3~gender+educ5+age4+income4, design=survey_123, family=quasibinomial(), subset = task=="cd")

logit_demo_task_ab_correct <- svyglm(correct~gender+educ5+age4+income4, design=survey_123, family=quasibinomial(), subset = task=="ab")
logit_demo_task_ab_correct_same <- svyglm(correct3~gender+educ5+age4+income4, design=survey_123, family=quasibinomial(), subset = task=="ab")
...
```{r demographics-table}
 coefs_clean <- function(dframe) {
  dframe %>% mutate(
    variable = c("Intercept", "Gender", rep("Education", 4), rep("Age", 3), rep("Income", 3)),
