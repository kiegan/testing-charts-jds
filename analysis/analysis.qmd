...
dframe %>% 
  ...
  tidyr::pivot_wider(
        ...
        names_from = level,
        values_from = c(estimate, std.error, letters)
      ) %>%
  gt() %>%
  tab_header(title = "Log odds (SD) of accuracy by task and chart type") %>%
  fmt_number(
    everything(),
    decimals=2
  ) %>%
  cols_merge(
    columns = c("estimate_correct | same or wrong", "std.error_correct | same or wrong",
                "letters_correct | same or wrong"),
    pattern = "<<{1}>><< ({2})>><< {3}>>"
  ) %>%
  cols_merge(
    columns = c("estimate_correct or same | wrong", 
                "std.error_correct or same | wrong",
