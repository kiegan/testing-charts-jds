...
```{r data-1}
# other github repo. 
round1 <- readRDS(file.path(vs_repo_local, "visual-studies/data/round1.rds"))
round2 <- readRDS(file.path(vs_repo_local, "visual-studies/data/round2.rds"))
...
```{r data-rnd6}
# other github repo. 
round6 <- readRDS(file.path(vs_repo_local, "visual-studies/data/round6.rds"))
...
```{r data-rnd7}
# other github repo. 
round7 <- readRDS(file.path(vs_repo_local, "visual-studies/data/round7.rds"))
...
round_12 <- round1_adj %>% 
            ...
            select(CaseId, ab, cd, CorrectAB, CorrectCD, CorrectABCD,
...
# ordering of levels in Response is from extremely=1 to not at all=5 - not very intuitive
...
Using linear scores for the response of `Certainty`, with `not certain at all` assigned a score of 1 and `extremely certain` assigned a score of 5, we can estimate the effects of task and correctness on certainty  by using  a general normal linear model of the form:
$$
Y_{k} = \mu + \tau_{i(k)} + \gamma_{j(k)} + \epsilon_{k},
$$
where $k = 1, ..., N$, $\mu$ is average certainty (measured on a scale from 1 to 5), $\tau_{i(k)}$ is the task in the $k$th response (with $i(k) = 1$ for unaligned and $i(k)=2$ for aligned comparisons) and $\gamma_{j(k)}$ is the correctness of the $k$th response (with $j(k) = 1$ for a correct and $j(k)=2$ for a wrong answer). We also assume that errors are normally distributed, i.e. $\epsilon_k \stackrel{i.i.d}{\sim} N(0, \sigma^2)$ for all $k = 1, ..., N$. We use a constraint of first effects as zero to make effects estimable, i.e. $\widehat{\mu}$ has to be interpreted as the average certainty of participants who are wrong on the unaligned comparison.

```{r model-certainty}
model_certain <- svyglm(as.numeric(Response)~Question+Correct, design = survey_12_crt)

coefs_certainty <- broom::tidy(model_certain)

coefs_certainty$term <- c("$$\\widehat{\\mu}$$", "$$\\widehat{\\tau}_2$$", "$$\\widehat{\\gamma}_2$$")
...
The four labelled pieces A, B, C, and D correspond to values 46, 47.6, 45.2, and 44.4, respectively, i.e underlying each evaluation is the order D < C < A < B.

```{r}
#| label: fig-tasks-67
#| fig-cap: "Stimuli shown to panelists. For each of the charts, all marked pieces were evaluated against each other. "
#| fig-height: 3
#| fig-width: 4
#| layout-nrow: 1
#| fig-subcap:
...
# D < C < A < B.
response_67 <- rbind(
    response_6 %>% select(CaseId, labelled, task, responses, weight),
    response_7 %>% select(CaseId, labelled, task, responses, weight)
  ) %>% mutate(
...
```{r model-67}
# still putzing around with it
logit_67_correct <- svyglm(correct~task+labelled + task, family=quasibinomial(), design=survey_67)

logit_67_cont <- svyglm(correct~task*signal, family=quasibinomial(), design=survey_67)
