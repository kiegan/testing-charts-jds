...
@tbl-designs shows the three designs that were considered here: We are using the **vertical** design as a baseline and have established that we get similar responses in different rounds of the survey. This design also produces stable results under slight modifications, such as the exact color scheme.

```{r}
#| label: tbl-designs
#| tbl-cap: Design and stimulus of round 3
#| tbl-subcap:
...
We are mostly interested in a comparison between this vertical design and the **wide horizontal** design favored by many NORC designers (XXX can I say that this way?). This design *should* help two-fold with decision making, because a. the bars are overall wider, increasing the physical representation of the stimulus, b. the bars are closer together. Both of these factors have been shown [@lu2022] to be beneficial for an accurate assessment of small differences between bars.

In order to separate these two effects from the effect of horizontal vs vertical assessment of bars, we introduced the **horizontal** design as a transition, in which the bars are kept to the exact same size as in the vertical design. Similarly, the spacing of the bars is identical between these two designs.

Interestingly, the resulting effects on the accuracy of the responses are not completely straightforward: the shift from a horizontal to a vertical design seems to be detrimental to an accurate perception in both situations. The re-scaled design of the wide horizontal bars reclaims some of the loss for unaligned bars and outperforms the vertical design by a similar margin in aligned bars. A contributing factor to this outcome might be the way that participants interact with the different designs. Generally, about half of all participants make use of the option to zoom into charts - and it can be shown that zooming into charts helps with the overall accuracy (which is in agreement with the findings by @lu2022 about the physical size of stimuli). The rate of zooming into charts is about twice as high for participants filling out the survey on their smart phones than on other, usually bigger devices such as tablets or desktops. However, different designs lead to different rates of zooming: when dealing with the vertical design, the rate of zooming is higher than for the two horizontal designs.
