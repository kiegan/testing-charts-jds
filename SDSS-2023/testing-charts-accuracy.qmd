...
\kr{Can we update the figure and t-test with combined data across all tasks/all rounds 1-3?}\hh{done}
...
\kr{ Can we update this to include the values for ROund 3 as well?}\hh{done}
...
The resulting $t$-statistic is highly significant ($t$ statistic: `r round(paired$statistic,1)`, df: `r paired$parameter`, $p$-value: \< 2.2e-16). \hh{how about: $p$-value: $<< 0.0001$ -- the e-16 looks very computery}
...
\kr{We need to update this chart to include the data from all 3 rounds instead of just the first 2} \hh{done}
...
A contributing factor to the observed patterns in response accuracy might be the way that participants interact with the different designs.
...
I'll pretty up @tbl-zoom-123 if we are going to keep it. \kr{I think we should! it's nice to have a model for this}
