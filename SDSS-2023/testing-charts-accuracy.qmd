...
survey1 <- survey::svydesign(
  ...
  ids = round1$CaseId, weights = round1$weight)

survey2 <- survey::svydesign(
  data = round2 %>% 
            filter(ab != "SKIPPED ON WEB", cd != "SKIPPED ON WEB"), 
  ids = round2$CaseId, weights = round2$weight)
...
```{r}
round_n1 <- nrow(round1)
round_n2 <- nrow(round2)
# effective sample size n/D, where D is the design effect
d1 <- deff(survey1)
```
