...
```
What we call 'aligned' and 'unaligned', here, is similar to Cleveland and McGill's set of rankings, but with some modifications: both 'aligned' and 'unaligned' bars (in tests 1 and 2) or wedges (in test 3) share the same axis.
...
\autoref{fig-tasks-12} shows the two stacked bar charts shown to participants in the first task.
...
Both charts in the left column of \autoref{fig-tasks} show the same data with slight modifications to the order of the levels -- the first and second level in each of the bars are reversed between the left and the right chart.
...
Task 1 was presented to viewers across two distinct rounds, with four color scheme variations each presented to 50% of respondents in each round.
...
```{r}
#| label: fig-tasks
#| fig-cap: "Each of the stimuli presented to viewers. In each bar chart, two pieces are marked. The larger piece in each chart is piece A or C. "
#| fig-subcap:
#| - "**Vertical stacked bar, unaligned comparison**"
#| - "**Horizontal stacked bar, unaligned comparison**"
#| - "**Wide horizontal stacked bar, unaligned comparison**"
#| - "**Vertical stacked bar, aligned comparison**"
#| - "**Horizontal stacked bar, aligned comparison**"
#| - "**Wide horizontal stacked bar, aligned comparison**"
#| layout-nrow: 2
#| out-width: 100%
...
Once again Tasks 2 and 3 were asked within the same survey round and the full sample was randomly split among respondents, with 50% of viewers seeing the Task 2 stimuli and 50% of viewers seeing the Task 3 stimuli.
...
Story here: not a huge difference, but we mainly see a difference in how people react.
horizontal/vertical/horizontal.
Even if there's not hugely significant differences by design, we can see differences in behavior.

Combine Tasks 1 and 2 and just present overall, aligned/unaligned by the 3 different orientations?
Doesn't matter whether we have a vertical or horizontal position,

We can reproduce We can rank We can rank better because we have a bigger sample size WE can find changes in viewer behavior Demographics matter to accuracy and choices
...
Interestingly, while we should, theoretically, see an improvement in accuracy when shifting from the vertical to the horizontal wide design, the resulting effects on the accuracy of the responses are not completely straightforward: The shift from a vertical to the (tall) horizontal design is detrimental to an accurate perception for both aligned and unaligned comparisons.\
