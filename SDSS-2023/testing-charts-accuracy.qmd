...
A data chart -- at its core -- maps values to graphical elements: quantitative elements are represented as measurable features, such as position, size, or shade. \hh{do we need to also go into qualitative variables?}
...
Based on mappings of quantitative variables to different graphical elements \ndt{example of element to explain it XXX}, Cleveland and McGill's study resulted in a ranking of perceptual tasks \ndt{give example of task} from most accurate to least accurate, which was then extended by  @mackinlayAutomatingDesignGraphical1986 to a theoretical framework ranking tasks' order along their ordinal and nominal scales.
...
The current panel size is 54,001 panel members aged 13 and over residing in over 43,000 households [@dennis2019technical]. \ejm{46,565 panelists are CAWI; they are 93.3% of the panel and 93.6% of the total panel weights.}
...
When combining responses from different surveys, weights \ejm{are rescaled} with respect to the total population, so that their order after combining still reflects their relative importance. 
...
| Task 3b | Floating bars |`r n5` | `r sprintf("%.1f",n5_eff)` |
