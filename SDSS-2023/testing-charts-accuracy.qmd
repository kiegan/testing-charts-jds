...
#| fig-cap: Comparisons made in charts across tasks 1 through 3 within the Cleveland and McGill ranking 
...
### Task 1: Alignment in stacked bar charts
...
### Task 2: Orientation of stacked bar charts

Story here: not a huge difference, but we mainly see a difference in how people react. horizontal/vertical/horizontal. Even if there's not hugely significant differences by design, we can see differences in behavior. 

Combine Tasks 1 and 2 and just present overall, aligned/unaligned by the 3 different orientations? Doesn't matter whether we have a vertical or horizontal position, 
...
In \autoref{fig-tasks-3} we see two structural variations on the stacked bar chart seen in Task 1: first, the Task 1 chart sized equally but rotated 90 degrees with bar lengths shown horizontally.
Second, the horizontal chart with a smaller aspect ratio so the bars appear thinner and longer.
Note that the wide version of the horizontal bars increases the pixel difference between the two marked pieces relative to the standard versions of the horizontal and vertical bars.

This test utilized a split sample approach, where 50% of participants in the sample were shown the tall horizontal bar chart and 50% were shown the wide horizontal chart.
Participants were asked to complete the same task as in Test 1; selecting the correct response among:
...
### Task 3: Alignment and orientation in pie charts
...
The current panel size is 54,001 panel members aged 13 and over residing in over 43,000 households **CITE DENNIS 2022** @dennis2019technical. \hh{I don't think I've got the right citation}
...
Note that @omuircheartaighCombiningSamplesVs2002 estimate $\lambda$ separately for any combination of race/ethnicity by sex. We employ that strategy whenever we include demographic variables in the analysis, otherwise we will use a single adjustment for the weights.
...
We combine rounds 1, 2 and 3 as shown in Table above to incorporate all relevant responses to evaluate the three different designs of charts in terms of their accuracy in assessing small differences in tile sizes. 
...
#### Viewers react differently to the charts
