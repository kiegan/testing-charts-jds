...
\kr{Do we have a measurement of the pixels for this? I think a sentence on that would close out this section nicely.}
...
round_123 <- round_123 %>% group_by(round) %>% 
  mutate(
...
```{=tex}
\hh{@tbl-demographics doesn't survive the formatting to pdf very well - need to shorten the row names or put into multiple lines} \hh{table rotates in pdf}
```
