...
\hh{XXX I think what you are trying to say is that we do not know, if participants have seen the stimuli multiple times, but if they did, there was at least a month in between?}
...
-   When combining responses from multiple rounds, we make weighting adjustments to ensure that weights in each sample are properly calibrated,  to the population total across both rounds.
...
- (some) Demographics matter: age and gender does not seem to matter for perception
  economic background and education does matter for perception: we need to know, we do not want to create a hurdle in communication 
